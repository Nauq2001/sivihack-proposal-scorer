...
## Data and access
The import process will run regularly to keep the dashboard current. A status panel will indicate when an update has completed, and users will be able to contact support if figures appear unusual. Our team will discuss data validation priorities with [ENTITY] during discovery and incorporate the agreed rules into the configuration.

Historical information will be imported from the supplied CSV files after we review their format and suitability. We will prepare a summary of the import outcome for the project owner. Meter names and property groupings will follow the mapping provided by [ENTITY], with ambiguous records referred back to the operations team.
